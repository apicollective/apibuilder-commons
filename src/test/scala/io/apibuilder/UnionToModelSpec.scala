...
import io.apibuilder.commons.util.{UnionModel, UnionToModel}
import io.flow.test.utils.FlowPlaySpec
import org.scalatest.Assertion

class UnionToModelSpec extends FlowPlaySpec
  ...
  with MultiServiceHelpers
{
  private[this] def setup(
    ...
    UnionToModel(makeMultiService(service)).toModel(ApiBuilderType.Union(service, union))
  }
  ...
  "validates all types are models" must {
    "enum" in {
      ...
      setupInvalid(enums = Seq(enum)) must equal(
        Seq("Union type 'test' is an enum. A model is required")
      )
    }
    ...
    "union" in {
      ...
      setupInvalid(unions = Seq(union)) must equal(
        Seq("Union type 'test' is a union. A model is required")
      )
    }
  }
