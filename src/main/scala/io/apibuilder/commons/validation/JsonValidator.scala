...
import io.apibuilder.commons.types.{ApiBuilderService, ApiBuilderType, Booleans}
...
object JsonValidator {
  def apply(service: Service): JsonValidator = apply(List(ApiBuilderService(service)))
  def apply(service: ApiBuilderService): JsonValidator = apply(List(service))
...
case class JsonValidator(validator: ValidatedJsonValidator) {
  import io.apibuilder.commons.validation.util.Implicits._
...
object ValidatedJsonValidator {
  def apply(service: Service): ValidatedJsonValidator = apply(ApiBuilderService(service))
...
case class ValidatedJsonValidator(services: List[ApiBuilderService]) {
  ...
  private[this] def findType(service: ApiBuilderService, typeName: String): Option[ApiBuilderType] = {
    service.service.enums.find(_.name.equalsIgnoreCase(typeName)) match {
      case Some(e) => Some(ApiBuilderType.Enum(service, e))
      case None => {
        service.service.models.find(_.name.equalsIgnoreCase(typeName)) match {
          case Some(m) => Some(ApiBuilderType.Model(service, m))
          case None => {
            service.service.unions.find(_.name.equalsIgnoreCase(typeName)) match {
